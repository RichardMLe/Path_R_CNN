...
class ProposalLayer(KE.Layer):
    ...
    box refinment deltas to anchors.
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    # Height and width are in feature map space
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    # Random horizontal, vetical flips and 90 degree rotation.
    if augment:
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Generate Anchors in one array [N, (y1, x1, y2, x2)]
        ...
        layer_outputs = []  # list of RPN output
        ...
        if mode == "training":
            ...
            # mrcnn_mask [batch, rois_size, ROI_pool * 2, ROI_pool * 2, num_classes]
    ...
    def find_specific(self, epoch, **kwargs):
        """Finde the checkpoint file specified by epoch and held_out_set (optional) 
        ...
        Returns:
            ...
            checkpoint_path: the path to the last checkpoint file
        """
        # Get directory names with optional
        held_out_set = kwargs.get("held_out_set", -1);
        ...
        if (held_out_set >= 0):
            dir_names = filter(lambda f: f.endswith(str(held_out_set)), dir_names)
            dir_names = sorted(dir_names)
        
        if not dir_names:
            return None, None
        # Pick last directory that satisfy the requirement
        dir_name = os.path.join(self.model_dir, dir_names[-1])
        # Find the specified checkpoint
        checkpoints = next(os.walk(dir_name))[2]
        checkpoints = filter(lambda f: f.startswith("mask_rcnn"), checkpoints)
        checkpoints = sorted(checkpoints)
        checkpoints = filter(lambda f: f.endswith(str(epoch).zfill(4) + '.h5'), checkpoints)
        checkpoints = sorted(checkpoints)
        if not checkpoints:
            return dir_name, None
        checkpoint = os.path.join(dir_name, checkpoints[-1])
